  ...
  updateCampaignStatus: asyncHandler(async (req, res) => {
    ...
    if (status === "active") {
      void startCampaignExecution(campaign.id).catch((err) => {
        console.error(`[Campaign ${campaign.id}] Error from detached startCampaignExecution (updateStatus):`, err);
        storage.updateCampaign(campaign.id, { status: "failed" }).catch(() => {});
      });
    }
  }),
  ...
  if (carouselCards) {
    ...
    for (let cardIdx = 0; cardIdx < carouselCards.length; cardIdx++) {
      ...
      if (cardComponents.length === 0) {
        console.warn(
          `[buildContactComponents] Carousel card ${cardIdx} has no components — ` +
          `no media ID found (campaignMediaId=${campaignMediaId}, card.mediaUrl=${card.mediaUrl}) ` +
          `and no body variables. Meta will likely reject this payload.`
        );
      }
...
async function _runCampaignQueuePopulation(campaignId: string, campaignData: any) {
  ...
  for (let i = 0; i < contacts.length; i += CHUNK_SIZE) {
    ...
    console.log(`[Campaign ${campaignId}] Queued ${totalQueued}/${contacts.length}`);
  }
