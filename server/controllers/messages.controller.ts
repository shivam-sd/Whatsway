  ...
  if (fromUser) {
    ...
    try {
      ...
      if (templateName) {
        ...
        const template = await storage.getTemplatesByName(templateName);
        msgBody = template[0]?.body || `[template: ${templateName}]`;
      ...
      // MEDIA MESSAGE
        ...
        // Determine full URL for WhatsApp
        const baseUrl = process.env.BASE_URL || 'http://localhost:5001';
        const fileUrl = file.cloudUrl
          ? `${baseUrl}/uploads/${file.cloudUrl}`   // cloud or local URL
          : `${baseUrl}/uploads/${file.filename || file.originalname}`;

        console.log(`📤 Processing media for WhatsApp: ${fileUrl}`);
...
if (file.size > MAX_SIZE_MB * 1024 * 1024) {
  throw new Error(`❌ ${file.originalname} exceeds WhatsApp size limit (${MAX_SIZE_MB}MB).`);
        ...
        try {
          ...
          console.warn("⚠️ Failed to get WhatsApp media URL, using local/cloud URL instead");
          mediaUrl = fileUrl;
        }
...
export const sendMessage = asyncHandler(async (req: RequestWithChannel, res: Response) => {
  ...
  // ================= CONTACT (IMPORTANT) =================
  let contact =
    (await storage.getContactByPhone(to)) ||
  ...
  if (templateName) {
    ...
    // ✅ SEND TEMPLATE (ONLY STRINGS GO TO WHATSAPP)
...
export const sendMessageeee = asyncHandler(async (req: RequestWithChannel, res: Response) => {
    console.log("📦 /api/messages/send BODY =>", JSON.stringify(req.body, null, 2));
  ...
  if (templateName) {
    ...
    console.log("Template sent, result:", mediaId);
    ...
    // Handle media upload + send
    console.log("CHECKKKKKKKK !!!!!!", mediaId)
    const mimeType = file.mimetype;
